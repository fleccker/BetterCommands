    ...
    public class CollectionParser : ICommandArgumentParser
    {
        ...
        public IResult Parse(string value, Type type)
        {
            if (type.IsArray)
            {
                ...
                if (!CommandArgumentParser.TryGetParser(elementType, out var parser)) return new ErrorResult($"Element of type {elementType.FullName} does not have a registered parser.");

                if (!TrySplitList(value, out var split)) return new ErrorResult($"Failed to parse array: failed to split string.");
                ...
                foreach (var val in split)
                {
                    ...
                    else list.Add(valResult.Result);
                }
            ...
            else if (type.IsSubclassOf(typeof(IDictionary<,>)))
            {
                ...
                if (!CommandArgumentParser.TryGetParser(keyType, out var keyParser) || !CommandArgumentParser.TryGetParser(valType, out var valParser)) return new ErrorResult($"Either the key or value type of {type.FullName} is not supported!");
                ...
                foreach (var val in split)
                {
                    if (!TrySplitPair(val, out var pairs)) return new ErrorResult($"Failed to split value {val} into a pair!");
                    ...
                    if (keyRes is ErrorResult) return keyRes;
                    if (valRes is ErrorResult) return valRes;
            ...
            else if (type.IsSubclassOf(typeof(IEnumerable<>)))
            {
                ...
                if (!CommandArgumentParser.TryGetParser(elementType, out var parser)) return new ErrorResult($"Failed to retrieve a parser for element type: {elementType.FullName}");
                if (!TrySplitList(value, out var values)) return new ErrorResult($"Failed to parse collection: failed to parse string into a list.");
                ...
                foreach (var val in values)
                {
                    ...
                    if (valRes is ErrorResult) return valRes;
                    else list.Add(valRes.Result);
                }

                return new SuccessResult(MakeCollection(list, type));
            }
            else return new ErrorResult($"An unsupported collection type was passed to the collection parser.");
        }

        private static object MakeArray(object[] array, Type type)
        {
            var arrayType = Activator.CreateInstance(type) as Array;
            for (int i = 0; i < array.Length; i++) arrayType.SetValue(array[i], i);
            return arrayType;
        }
        ...
        private static object MakeDictionary(IEnumerable<KeyValuePair<object, object>> pairs, Type dictType, Type keyType, Type valType)
        {
            ...
            foreach (var pair in pairs) dict[pair.Key] = pair.Value;
        ...
        private static object MakeCollection(IEnumerable<object> list, Type collectionType)
        {
            ...
            else return constructor.Invoke(new object[] { list });
        }

        private static Type[] GetElementType(Type collectionType, bool generics = false)
        {
            if (collectionType.IsArray) return new Type[] { collectionType.GetElementType() };
            else if (generics) return collectionType.GetGenericArguments();
            else return null;
        }

        private static bool TrySplitList(string value, out string[] values)
        {
            if (value.Count(x => x == ':') == 1) values = value.Split(':');
            else if (value.Count(x => x == ';') == 1) values = value.Split(';');
        ...
        {
            if (value.Count(x => x == ':') == 1) pairs = value.Split(':');
            else if (value.Count(x => x == '=') == 1) pairs = value.Split('=');
            else if (value.Count(x => x == ';') == 1) pairs = value.Split(';');
            else pairs = value.Split(' ');
